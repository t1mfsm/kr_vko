#!/bin/bash
# Скрипт запуска всех элементов системы ВКО
# Использование: ./start.sh [компонент]
# Без аргументов - запуск всех систем
# С аргументом: gen, kp, rls1, rls2, rls3, zrdn1, zrdn2, zrdn3, spro
...
start_component() {
    ...
    case "$component" in
        ...
        kp)
            ...
            ;;
        rls1)
            echo "[*] Запуск РЛС1 (Воронеж-ДМ, Кишинев)..."
            bash "$SCRIPT_DIR/rls.sh" 1 &
            echo "[+] РЛС1 запущена"
            ;;
        rls2)
            echo "[*] Запуск РЛС2 (Дарьял)..."
            bash "$SCRIPT_DIR/rls.sh" 2 &
            echo "[+] РЛС2 запущена"
            ;;
        rls3)
            echo "[*] Запуск РЛС3 (Днепр, Иркутск)..."
            bash "$SCRIPT_DIR/rls.sh" 3 &
            echo "[+] РЛС3 запущена"
            ;;
        zrdn1)
            echo "[*] Запуск ЗРДН1 (Оренбург)..."
            bash "$SCRIPT_DIR/zrdn.sh" 1 &
            echo "[+] ЗРДН1 запущен"
            ;;
        zrdn2)
            echo "[*] Запуск ЗРДН2 (Волгоград)..."
            bash "$SCRIPT_DIR/zrdn.sh" 2 &
            echo "[+] ЗРДН2 запущен"
            ;;
        zrdn3)
            echo "[*] Запуск ЗРДН3 (Махачкала)..."
            bash "$SCRIPT_DIR/zrdn.sh" 3 &
            echo "[+] ЗРДН3 запущен"
            ;;
        spro)
            echo "[*] Запуск СПРО (Омск)..."
